...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $basePrice = round(($price / $taxPercent) * 100, 2);
...
//        $data['shipping'] = 0;
    ...
    {
        $cartsItems = collect($request->get('carts'));
        $cartIds = $cartsItems->pluck('id');
        $carts = Cart::whereIn('id', $cartIds->toArray())->get();
        if ($request->get('payment_option') === 'cash_on_delivery') {
            $carts->each(
                function ($cart) {
                    $cart->update(['shipping_cost' => 50]);
                }
            );
        } else {
            $carts->each(
        ...
        return view(
            'frontend.partials.cart_summary',
            [
                'carts' => $carts,
